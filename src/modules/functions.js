...
export function finalPage() {
  ...
  const newTotalScore = document.createElement('p');
  newTotalScore.setAttribute('id', 'ivt');
  newTotalScore.classList.add('invite-wrapper', 'invite');
  newTotalScore.textContent = `You scored ${totalScore} points out of 30`;
  const totalLink = document.createElement('a');
  totalLink.classList.add('center', 'center-final');
  totalLink.setAttribute('href', './example.html');
  totalLink.textContent = 'Play again';
  textContent.append(newTotalScore, totalLink);
    ...
    item.addEventListener('click', (e) => {
      if (e.target !== e.currentTarget) {
        ...
        if (
          ...
          score.innerHTML = `Score: ${(totalScore += numCount(
            sumPointsArray.length
          ))}`;
          let newContent = document.querySelector('.chouse-birds');
          document.querySelector('.chouse-birds').innerHTML = '';
          newContent.innerHTML = `
            <div class="new-content">
            <img class="new-image" src=${birdsNewImage(
              birdsDataEn,
              numArray,
              e.currentTarget.innerText
            )} alt=${e.currentTarget.innerText}>
            <h3 class="new-h3">${e.currentTarget.innerText}</h3>
            <h4 class="new-h4">${birdsLatinoDesc(
              birdsDataEn,
              numArray,
              e.currentTarget.innerText
            )}</h4>
            <audio controls>
              <source src="${birdsAudio(
                birdsDataEn,
                numArray,
                e.currentTarget.innerText
              )}" type="audio/mp3">
            </audio>
            <p class="new-desc">${birdsDesc(
              birdsDataEn,
              numArray,
              e.currentTarget.innerText
            )}</p>
